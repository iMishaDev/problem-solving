...
class Solution {
    find_min_cost(days, costs){
        return this.find_min_cost_helper(days, costs)
    }
    ...
    find_min_cost_helper(days, costs){
        ...
        return (function find_min_cost_internal(day){
            ...
            for(const [i, cost_days] of [1, 7, 30].entries()){
                let d = day
                while(d < days.length && days[d] < days[day] + cost_days){
                    d += 1;
                }
...
console.log(new Solution().find_min_cost([1,4,6,7,8,20], [2,7,15]))